...
Bring the repository from the current development state to a continuously tested, recoverable, secure, provider-agnostic production architecture while preserving the offline/mock-first design.

## Execution order

### Phase 0 — CI and API baseline
- Fix FastAPI startup/test-collection failures.
...
- Audit all API v1 request/response models against the factory pipeline contract.
...
- Add deterministic integration tests for STORY → SCENE → SHOT and future media stages.
...
- Ensure local/mock providers work without external credentials.
- Add timeout/fallback behavior without hiding real provider failures.

### Phase 5 — Asset, provenance, and storage integrity
- Audit content-addressed storage and checksum verification.
- Ensure every generated asset records provider/model/prompt/job lineage.
...
- Implement deterministic best-take scoring and invalid-take rejection.
- Harden timeline validation and renderer job inputs.
- Add FFmpeg process isolation, timeouts, output validation, and resource limits.
...
- Add offline/mock development configuration.
...
2. Full mock pipeline runs end-to-end without external APIs or paid services.
