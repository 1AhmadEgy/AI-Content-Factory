...
object NetworkClient {
    ...
    private fun normalizedBaseUrl(): String {
        ...
        val baseUrl = if (configured.isBlank() || configured == NOT_CONFIGURED) {
            check(BuildConfig.DEBUG) { "AICF_API_BASE_URL is required for release builds. Configure it in CI." }
            DEFAULT_EMULATOR_BASE_URL
        } else configured
        require(baseUrl.startsWith("https://") || (BuildConfig.DEBUG && baseUrl.startsWith("http://"))) {
            "Backend URL must use HTTPS outside debug builds."
        }
    ...
    private val requestInterceptor = Interceptor { chain ->
        ...
        val token = BuildConfig.AICF_API_KEY.trim()
