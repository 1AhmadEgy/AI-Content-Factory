...
fun MainScreen() {
    ...
    Scaffold(topBar = { if (route != "designer") BrandingHeader() }, snackbarHost = { SnackbarHost(snackbarHostState) }, bottomBar = {
        ...
        NavHost(navController, startDestination = "projects", modifier = Modifier.padding(padding)) {
            ...
            composable("series/{seriesId}", arguments = listOf(navArgument("seriesId") { type = NavType.StringType })) { e -> val id = e.arguments?.getString("seriesId") ?: return@composable; SeriesDetailScreen(id, viewModel, { navController.popBackStack() }, { navController.navigate("episode/$it") }) }
            ...
            composable("episode/{episodeId}", arguments = listOf(navArgument("episodeId") { type = NavType.StringType })) { e -> val id = e.arguments?.getString("episodeId") ?: return@composable; EpisodeDetailScreen(id, viewModel, { navController.popBackStack() }) }
