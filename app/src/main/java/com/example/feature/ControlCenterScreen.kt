                ...
                ControlCenterJobCard(
                    ...
                    onCancel = {
                        scope.launch {
                            ...
                            catch (t: Throwable) { error = t.message ?: "فشل الإلغاء" }
                        }
                    },
                    onRetry = {
                        scope.launch {
                            try { NetworkClient.apiService.retryJob(job.id); refresh() }
                            catch (t: Throwable) { error = t.message ?: "فشلت إعادة المحاولة" }
                        }
                    },
                )
            }
...
private fun ControlCenterJobCard(
    ...
    Card(modifier = Modifier.fillMaxWidth(), onClick = onSelect) {
        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            if (selected) {
                Text("تشغيلات المزود (${runs.size})", style = MaterialTheme.typography.labelLarge)
