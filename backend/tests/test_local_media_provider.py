...
class _Response:
    def __init__(self, body: bytes, content_type: str = "application/json", run_id: str | None = "run-123") -> None:
        self._body = body
        self.headers = {"Content-Type": content_type}
        if run_id is not None:
            self.headers["X-Provider-Run-Id"] = run_id
...
def test_local_media_adapter_rejects_raw_binary_without_provider_run_id(monkeypatch):
    monkeypatch.setattr("backend.app.providers.local_media.urlopen", lambda *args, **kwargs: _Response(b"VIDEO-DATA", "video/mp4", run_id=None))
    adapter = LocalMediaModelAdapter("http://127.0.0.1:9000/generate", frozenset({"video"}))
    response = adapter.execute(ProviderRequest(model="local-video", parameters={"duration": 4}))
    assert response.success is False
    assert response.error_code == "LOCAL_MEDIA_RUN_ID_MISSING"
...
def test_local_media_adapter_rejects_json_without_provider_run_id(monkeypatch):
    payload = {"data": base64.b64encode(b"PNG-DATA").decode(), "mime_type": "image/png"}
    monkeypatch.setattr("backend.app.providers.local_media.urlopen", lambda *args, **kwargs: _Response(json.dumps(payload).encode(), run_id=None))
    adapter = LocalMediaModelAdapter("http://127.0.0.1:8188/generate", frozenset({"image"}))
    response = adapter.execute(ProviderRequest(model="local-image", parameters={"prompt": "test"}))
    assert response.success is False
    assert response.error_code == "LOCAL_MEDIA_RUN_ID_MISSING"
