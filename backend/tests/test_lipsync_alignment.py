...
def test_align_words_applies_offset_and_preserves_provenance():
    result = LipSyncAlignmentEngine().align_words([{"text":"Hello","start_ms":100,"end_ms":500,"confidence":0.99,"shotId":"shot-1"},{"text":"world","start_ms":520,"end_ms":900,"confidence":0.95}], duration_ms=1500, offset_ms=50, provenance={"provider_id":"real-asr","provider_run_id":"run-1"})
...
def test_build_subtitles_groups_words_without_exceeding_limits():
...
def test_low_confidence_words_can_be_filtered():
    result=LipSyncAlignmentEngine().align_words([{"text":"keep","start_ms":0,"end_ms":300,"confidence":0.9},{"text":"drop","start_ms":400,"end_ms":700,"confidence":0.2}],duration_ms=1000,minimum_confidence=0.8)
    assert [word.text for word in result.words] == ["keep"]

def test_sync_score_is_thresholded_and_deterministic():
...
def test_invalid_timing_is_rejected_instead_of_clamped():
...
def test_empty_expected_sync_is_rejected():
