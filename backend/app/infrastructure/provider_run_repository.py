...
from ..domain.repositories import ProviderRunRepository
...
class SQLiteProviderRunRepository(ProviderRunRepository):
    ...
    def __init__(self, store: SQLiteStore) -> None:
        ...
        with store._lock, store.connection:
            ...
            store.connection.execute("CREATE INDEX IF NOT EXISTS idx_provider_runs_status ON provider_runs(status, created_at)")

    @staticmethod
    def _row_to_model(row) -> ProviderRun:
        return ProviderRun(
    ...
    def list_for_job(self, job_id: str, limit: int = 50) -> list[ProviderRun]:
        ...
        with self.store._lock:
            rows = self.store.connection.execute(
                "SELECT * FROM provider_runs WHERE job_id=? ORDER BY created_at DESC,id DESC LIMIT ?", (job_id, limit)
            ).fetchall()
