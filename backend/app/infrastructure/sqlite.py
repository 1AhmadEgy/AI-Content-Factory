...
class SQLiteStore:
    ...
    def get_idempotency(self, key: str, operation: str) -> sqlite3.Row | None:
        with self._lock:
            return self._connection.execute(
                "SELECT * FROM idempotency_keys WHERE key = ? AND operation = ?",
                (key, operation),
            ).fetchone()
...
class SQLiteProjectRepository(ProjectRepository):
    ...
    def create(self, project: Project) -> Project:
        self.store._insert(
            "INSERT INTO projects(id,name,description,settings_json,created_at,updated_at) VALUES(?,?,?,?,?,?)",
            (project.id, project.name, project.description, _json(project.settings), _dt(project.created_at), _dt(project.updated_at)),
        )
        return project
    ...
    def get(self, project_id: str) -> Project | None:
        ...
        return Project(
            ...
            json.loads(row["settings_json"] or "{}"),
...
class SQLiteEpisodeRepository(EpisodeRepository):
    ...
    def create(self, episode: Episode) -> Episode:
        self.store._insert(
            ...
            (episode.id, episode.project_id, episode.title, _dt(episode.created_at), _dt(episode.updated_at)),
        )
        return episode
...
class SQLiteSceneRepository(SceneRepository):
    ...
    def create(self, scene: Scene) -> Scene:
        self.store._insert(
            "INSERT INTO scenes(id,episode_id,title,order_index,created_at) VALUES(?,?,?,?,?)",
            (scene.id, scene.episode_id, scene.title, scene.order_index, _dt(scene.created_at)),
        )
        return scene
    ...
    def get(self, scene_id: str) -> Scene | None:
        ...
        return Scene(
            row["id"], row["episode_id"], row["title"], row["order_index"],
...
class SQLiteShotRepository(ShotRepository):
    ...
    def create(self, shot: Shot) -> Shot:
        self.store._insert(
            "INSERT INTO shots(id,scene_id,order_index,prompt,created_at) VALUES(?,?,?,?,?)",
            (shot.id, shot.scene_id, shot.order_index, shot.prompt, _dt(shot.created_at)),
        )
        return shot
    ...
    def get(self, shot_id: str) -> Shot | None:
        ...
        return Shot(
            row["id"], row["scene_id"], row["order_index"], row["prompt"],
            _parse_dt(row["created_at"]),
        ) if row else None
...
def _job_from_row(row: sqlite3.Row) -> GenerationJob:
    return GenerationJob(
        id=row["id"], parent_job_id=row["parent_job_id"], project_id=row["project_id"],
        type=JobType(row["type"]), target_type=row["target_type"], target_id=row["target_id"],
        priority=row["priority"], status=JobStatus(row["status"]), progress=row["progress"],
        attempt=row["attempt"], max_attempts=row["max_attempts"], provider=row["provider"],
        model=row["model"], input=_job_input_from_dict(json.loads(row["input_json"])),
        ...
        completed_at=_parse_dt(row["completed_at"]), updated_at=_parse_dt(row["updated_at"]),
    )
...
class SQLiteJobRepository(JobRepository):
    ...
    def create_with_idempotency(
        ...
        *,
        ...
        """Atomically resolve the idempotency key and create the job if needed."""
        with self.store._lock:
            connection = self.store.connection
            connection.execute("BEGIN IMMEDIATE")
            try:
                existing = connection.execute(
                    "SELECT operation, request_fingerprint, resource_id FROM idempotency_keys WHERE key = ?",
                    (key,),
                ).fetchone()
    ...
    def update_if_current(
        ...
        with self.store._lock, self.store.connection:
            cursor = self.store.connection.execute(
                "UPDATE jobs SET parent_job_id=?,project_id=?,type=?,target_type=?,target_id=?,priority=?,status=?,progress=?,attempt=?,max_attempts=?,provider=?,model=?,input_json=?,output_json=?,error_code=?,error_message=?,created_at=?,started_at=?,completed_at=?,updated_at=? WHERE id=? AND status=? AND attempt=?",
                (
                    job.parent_job_id, job.project_id, job.type.value, job.target_type, job.target_id,
                    job.priority, job.status.value, job.progress, job.attempt, job.max_attempts,
                    job.provider, job.model, _json(_job_input_to_dict(job.input)),
                    _json(_job_output_to_dict(job.output)) if job.output else None,
                    job.error_code, job.error_message, _dt(job.created_at), _dt(job.started_at),
                    _dt(job.completed_at), _dt(job.updated_at), job.id, expected_status.value,
                    expected_attempt,
                ),
            )
