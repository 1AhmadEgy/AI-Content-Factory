...
class SQLiteStore:
    ...
    def initialize(self) -> None:
        with self._lock, self._connection:
            ...
            """)
            columns = {r["name"] for r in self._connection.execute("PRAGMA table_info(projects)").fetchall()}
    ...
    def update_if_current(self, job: GenerationJob, expected_status: JobStatus, expected_attempt: int) -> GenerationJob:
        ...
        with self.store._lock, self.store.connection:
            cursor = self.store.connection.execute(
                """UPDATE jobs SET parent_job_id=?,project_id=?,type=?,target_type=?,target_id=?,priority=?,status=?,progress=?,attempt=?,max_attempts=?,provider=?,model=?,input_json=?,output_json=?,error_code=?,error_message=?,created_at=?,started_at=?,completed_at=?,updated_at=?
