...
from ..domain.repositories import EpisodeRepository, IdempotencyResult, JobRepository, ProjectRepository, SceneRepository, ShotRepository
...
class SQLiteStore:
        ...
        with self._lock, self._connection:
            self._connection.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS scenes (
                    ...
                    title TEXT NOT NULL, order_index INTEGER NOT NULL, created_at TEXT NOT NULL
                );
                ...
                CREATE TABLE IF NOT EXISTS shots (
                    ...
                    order_index INTEGER NOT NULL, prompt TEXT NOT NULL, created_at TEXT NOT NULL
                );
                CREATE INDEX IF NOT EXISTS idx_shots_scene_order ON shots(scene_id, order_index);
                CREATE TABLE IF NOT EXISTS jobs (
                    id TEXT PRIMARY KEY, parent_job_id TEXT REFERENCES jobs(id) ON DELETE SET NULL,
                    project_id TEXT NOT NULL REFERENCES projects(id) ON DELETE CASCADE, type TEXT NOT NULL,
                    target_type TEXT NOT NULL, target_id TEXT, priority INTEGER NOT NULL, status TEXT NOT NULL,
                    progress REAL NOT NULL, attempt INTEGER NOT NULL, max_attempts INTEGER NOT NULL,
                    provider TEXT, model TEXT, input_json TEXT NOT NULL, output_json TEXT, error_code TEXT,
                    error_message TEXT, created_at TEXT NOT NULL, started_at TEXT, completed_at TEXT,
                    updated_at TEXT NOT NULL, lease_owner TEXT, lease_expires_at TEXT
                );
            ...
            if "settings_json" not in project_columns:
                self._connection.execute("ALTER TABLE projects ADD COLUMN settings_json TEXT NOT NULL DEFAULT '{}'")
            job_columns = {row["name"] for row in self._connection.execute("PRAGMA table_info(jobs)").fetchall()}
            ...
            if "lease_expires_at" not in job_columns:
                self._connection.execute("ALTER TABLE jobs ADD COLUMN lease_expires_at TEXT")
    ...
    def claim_idempotency(self, key: str, operation: str, fingerprint: str, resource_id: str) -> bool:
        with self._lock, self._connection:
            try:
                self._connection.execute("INSERT INTO idempotency_keys(key,operation,request_fingerprint,resource_id,created_at) VALUES(?,?,?,?,?)", (key, operation, fingerprint, resource_id, _dt(utc_now())))
                return True
...
class SQLiteProjectRepository(ProjectRepository):
    ...
    def create(self, project: Project) -> Project:
        self.store._insert("INSERT INTO projects(id,name,description,settings_json,created_at,updated_at) VALUES(?,?,?,?,?,?)", (project.id, project.name, project.description, _json(project.settings), _dt(project.created_at), _dt(project.updated_at)))
        return project
    ...
    def get(self, project_id: str) -> Project | None:
        ...
        if not row:
            return None
        return Project(row["id"], row["name"], row["description"] or "", json.loads(row["settings_json"] or "{}"), _parse_dt(row["created_at"]), _parse_dt(row["updated_at"]))
...
class SQLiteEpisodeRepository(EpisodeRepository):
    ...
    def create(self, episode: Episode) -> Episode:
        self.store._insert("INSERT INTO episodes(id,project_id,title,created_at,updated_at) VALUES(?,?,?,?,?)", (episode.id, episode.project_id, episode.title, _dt(episode.created_at), _dt(episode.updated_at)))
        return episode

    def get(self, episode_id: str) -> Episode | None:
        row = self.store._get("episodes", episode_id)
        return Episode(row["id"], row["project_id"], row["title"], _parse_dt(row["created_at"]), _parse_dt(row["updated_at"])) if row else None
...
class SQLiteSceneRepository(SceneRepository):
    ...
    def create(self, scene: Scene) -> Scene:
        self.store._insert("INSERT INTO scenes(id,episode_id,title,order_index,created_at) VALUES(?,?,?,?,?)", (scene.id, scene.episode_id, scene.title, scene.order_index, _dt(scene.created_at)))
        return scene

    def get(self, scene_id: str) -> Scene | None:
        row = self.store._get("scenes", scene_id)
        return Scene(row["id"], row["episode_id"], row["title"], row["order_index"], _parse_dt(row["created_at"])) if row else None
...
class SQLiteShotRepository(ShotRepository):
    ...
    def create(self, shot: Shot) -> Shot:
        self.store._insert("INSERT INTO shots(id,scene_id,order_index,prompt,created_at) VALUES(?,?,?,?,?)", (shot.id, shot.scene_id, shot.order_index, shot.prompt, _dt(shot.created_at)))
        return shot

    def get(self, shot_id: str) -> Shot | None:
        row = self.store._get("shots", shot_id)
        return Shot(row["id"], row["scene_id"], row["order_index"], row["prompt"], _parse_dt(row["created_at"])) if row else None
...
class SQLiteJobRepository(JobRepository):
    ...
    def create_with_idempotency(self, job: GenerationJob, *, key: str, operation: str, fingerprint: str) -> IdempotencyResult:
        with self.store._lock:
            connection = self.store.connection
            connection.execute("BEGIN IMMEDIATE")
            try:
                existing = connection.execute("SELECT operation, request_fingerprint, resource_id FROM idempotency_keys WHERE key = ?", (key,)).fetchone()
                if existing is not None:
                    connection.commit()
                    matches = existing["operation"] == operation and existing["request_fingerprint"] == fingerprint
                    return IdempotencyResult(job=None, existing_resource_id=existing["resource_id"] if matches else None, conflict=not matches)
                self._insert_job(connection, job)
                connection.execute("INSERT INTO idempotency_keys(key,operation,request_fingerprint,resource_id,created_at) VALUES(?,?,?,?,?)", (key, operation, fingerprint, job.id, _dt(utc_now())))
                connection.commit()
    ...
    def update_if_current(self, job: GenerationJob, expected_status: JobStatus, expected_attempt: int) -> bool:
        with self.store._lock, self.store.connection:
            cursor = self.store.connection.execute("UPDATE jobs SET parent_job_id=?,project_id=?,type=?,target_type=?,target_id=?,priority=?,status=?,progress=?,attempt=?,max_attempts=?,provider=?,model=?,input_json=?,output_json=?,error_code=?,error_message=?,created_at=?,started_at=?,completed_at=?,updated_at=? WHERE id=? AND status=? AND attempt=?", (job.parent_job_id, job.project_id, job.type.value, job.target_type, job.target_id, job.priority, job.status.value, job.progress, job.attempt, job.max_attempts, job.provider, job.model, _json(_job_input_to_dict(job.input)), _json(_job_output_to_dict(job.output)) if job.output else None, job.error_code, job.error_message, _dt(job.created_at), _dt(job.started_at), _dt(job.completed_at), _dt(job.updated_at), job.id, expected_status.value, expected_attempt))
    ...
    def list(self, *, project_id: str | None = None, status: JobStatus | None = None, limit: int = 50) -> list[GenerationJob]:
        ...
        with self.store._lock:
            rows = self.store.connection.execute(f"SELECT * FROM jobs{where} ORDER BY created_at DESC, id DESC LIMIT ?", (*params, limit)).fetchall()
        return [_job_from_row(row) for row in rows]
...
class SQLiteRepositories:
    def __init__(self, path: str | Path = ":memory:") -> None:
        ...
        self.episodes = SQLiteEpisodeRepository(self.store)
        self.scenes = SQLiteSceneRepository(self.store)
        self.shots = SQLiteShotRepository(self.store)
        self.jobs = SQLiteJobRepository(self.store)
