...
class SQLiteStore:
    ...
    @property
    def connection(self) -> sqlite3.Connection: return self._connection
...
class SQLiteProjectRepository(ProjectRepository):
    ...
    def get(self, project_id: str) -> Project | None:
        ...
        return Project(row["id"], row["name"], row["description"] or "", json.loads(row["settings_json"] or "{}"), _parse_dt(row["created_at"]), _parse_dt(row["updated_at"])) if row else None
...
class SQLiteJobRepository(JobRepository):
    ...
    def create(self, job: GenerationJob) -> GenerationJob:
        with self.store._lock, self.store.connection: self._insert_job(self.store.connection, job)
        return job
    def create_with_idempotency(self, job: GenerationJob, *, key: str, operation: str, fingerprint: str) -> IdempotencyResult:
        with self.store._lock:
            connection = self.store.connection; connection.execute("BEGIN IMMEDIATE")
            try:
                ...
                if existing is not None:
                    ...
                    existing_job = _job_from_row(connection.execute("SELECT * FROM jobs WHERE id = ?", (existing["resource_id"],)).fetchone()) if matches and connection.execute("SELECT 1 FROM jobs WHERE id = ?", (existing["resource_id"],)).fetchone() else None
                    connection.commit()
                    return IdempotencyResult(job=existing_job, existing_resource_id=existing["resource_id"] if matches else None, conflict=not matches, replayed=matches)
                ...
                connection.commit(); return IdempotencyResult(job=job)
            except Exception:
    ...
    def update_if_current(self, job: GenerationJob, expected_status: JobStatus, expected_attempt: int) -> bool:
        with self.store._lock, self.store.connection:
            cursor = self.store.connection.execute("UPDATE jobs SET parent_job_id=?,project_id=?,type=?,target_type=?,target_id=?,priority=?,status=?,progress=?,attempt=?,max_attempts=?,provider=?,model=?,input_json=?,output_json=?,error_code=?,error_message=?,created_at=?,started_at=?,completed_at=?,updated_at=? WHERE id=? AND status=? AND attempt=?", (job.parent_job_id, job.project_id, job.type.value, job.target_type, job.target_id, job.priority, job.status.value, job.progress, job.attempt, job.max_attempts, job.provider, job.model, _json(_job_input_to_dict(job.input)), _json(_job_output_to_dict(job.output)) if job.output else None, job.error_code, job.error_message, _dt(job.created_at), _dt(job.started_at), _dt(job.completed_at), _dt(job.updated_at), job.id, expected_status.value, expected_attempt)); return cursor.rowcount == 1
