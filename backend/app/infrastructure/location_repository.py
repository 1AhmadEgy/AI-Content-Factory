...
class SQLiteLocationRepository(LocationRepository):
    ...
    def _from_row(r):
        return LocationProfile(
            id=r["id"], project_id=r["project_id"], name=r["name"], aliases=tuple(json.loads(r["aliases_json"])),
            description=r["description"], geography=json.loads(r["geography_json"]), architecture=json.loads(r["architecture_json"]),
            environment=json.loads(r["environment_json"]), visual_style=json.loads(r["visual_style_json"]), lighting=json.loads(r["lighting_json"]),
            weather=json.loads(r["weather_json"]), time_of_day=r["time_of_day"], props=tuple(json.loads(r["props_json"])),
            rules=tuple(json.loads(r["rules_json"])), negative_constraints=tuple(json.loads(r["negative_constraints_json"])),
            reference_asset_ids=tuple(json.loads(r["reference_asset_ids_json"])), provider_location_id=r["provider_location_id"],
            metadata=json.loads(r["metadata_json"]), version=r["version"], created_at=datetime.fromisoformat(r["created_at"]),
    ...
    def create(self, location):
        ...
        with self.store._lock, self.store.connection:
            self.store.connection.execute(
                ...
                (
                    location.id, location.project_id, location.name, self._json(location.aliases), location.description,
                    self._json(location.geography), self._json(location.architecture), self._json(location.environment),
                    self._json(location.visual_style), self._json(location.lighting), self._json(location.weather), location.time_of_day,
                    self._json(location.props), self._json(location.rules), self._json(location.negative_constraints),
                    self._json(location.reference_asset_ids), location.provider_location_id, self._json(location.metadata), location.version,
    ...
    def get(self, location_id):
        r = self.store._get("locations", location_id)
    ...
    def list(self, *, project_id=None, query=None, limit=100):
        ...
        with self.store._lock:
            rows = self.store.connection.execute(
                f"SELECT * FROM locations{where} ORDER BY name COLLATE NOCASE LIMIT ?",
