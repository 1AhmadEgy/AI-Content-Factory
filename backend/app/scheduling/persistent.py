...
class SQLiteScheduleRepository:
    def __init__(self, store: SQLiteStore) -> None:
        ...
        with store._lock, store.connection:
            store.connection.execute("""CREATE TABLE IF NOT EXISTS schedules (
                id TEXT PRIMARY KEY, project_id TEXT NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
                operation TEXT NOT NULL, payload_json TEXT NOT NULL, run_at TEXT NOT NULL,
                cron TEXT, interval_seconds INTEGER, timezone_name TEXT NOT NULL DEFAULT 'UTC',
    ...
    def create(self, schedule: Schedule) -> Schedule:
        ...
        schedule.created_at = now; schedule.updated_at = now
        if schedule.next_run_at is None: schedule.next_run_at = schedule.run_at
        self.store._insert("INSERT INTO schedules(id,project_id,operation,payload_json,run_at,cron,interval_seconds,timezone_name,enabled,last_run_at,next_run_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (schedule.id,schedule.project_id,schedule.operation,_json(schedule.payload),_dt(schedule.run_at),schedule.cron,schedule.interval_seconds,schedule.timezone_name,int(schedule.enabled),_dt(schedule.last_run_at),_dt(schedule.next_run_at),_dt(schedule.created_at),_dt(schedule.updated_at)))
        return schedule

    def _row(self, row) -> Schedule | None:
    ...
    def list(self, project_id: str | None = None, enabled: bool | None = None, limit: int = 200) -> list[Schedule]:
        clauses=[]; params=[]
    ...
    def due(self, now: datetime, limit: int = 50) -> list[Schedule]:
        with self.store._lock:
            rows=self.store.connection.execute("SELECT * FROM schedules WHERE enabled=1 AND next_run_at IS NOT NULL AND next_run_at<=? ORDER BY next_run_at,id LIMIT ?",(_dt(now),max(1,min(limit,200)))).fetchall()
        return [self._row(r) for r in rows]

    def update(self, schedule: Schedule) -> Schedule:
        schedule.updated_at=datetime.now(timezone.utc)
        with self.store._lock, self.store.connection:
            self.store.connection.execute("UPDATE schedules SET operation=?,payload_json=?,run_at=?,cron=?,interval_seconds=?,timezone_name=?,enabled=?,last_run_at=?,next_run_at=?,updated_at=? WHERE id=?",
                (schedule.operation,_json(schedule.payload),_dt(schedule.run_at),schedule.cron,schedule.interval_seconds,schedule.timezone_name,int(schedule.enabled),_dt(schedule.last_run_at),_dt(schedule.next_run_at),_dt(schedule.updated_at),schedule.id))
        return schedule
...
def next_cron_time(start: datetime, expression: str, horizon_minutes: int = 366*24*60) -> datetime:
    cursor=start.replace(second=0,microsecond=0)+timedelta(minutes=1)
    for _ in range(horizon_minutes):
        if cron_matches(cursor, expression): return cursor
        cursor += timedelta(minutes=1)
    raise ValueError("CRON_NEXT_RUN_NOT_FOUND")


class PersistentScheduler:
    def __init__(self, repository: SQLiteScheduleRepository, enqueue: callable):
        self.repository=repository; self.enqueue=enqueue; self._lock=RLock()

    def tick(self, now: datetime | None = None, limit: int = 50) -> list[str]:
        ...
        for schedule in self.repository.due(now,limit):
            with self._lock:
                ...
                child=self.enqueue(current)
                created.append(child.id if hasattr(child,"id") else str(child))
                current.last_run_at=now
                if current.cron: current.next_run_at=next_cron_time(now,current.cron)
