...
class QualityControlWorker(Worker):
    """Run deterministic technical QC and emit a machine-readable report."""
    ...
    def execute(self, job: GenerationJob, context: WorkerContext) -> JobExecutionResult:
        ...
        valid = [c for c in checks if c["exists"] and c["ready"] and c["checksum"]]
        score = round(100.0 * len(valid) / len(checks), 2)
        report = {"jobId": job.id, "assetIds": asset_ids, "checks": checks, "score": score, "passed": score == 100.0}
        ...
        report_asset = Asset(
            ...
            provenance=build_provenance(job, source_asset_ids=asset_ids, metadata={"score": score, "passed": score == 100.0}, license_status=LicenseStatus.VERIFIED),
        )
        self.assets.create(report_asset)
        return JobExecutionResult(success=True, asset_ids=[report_id], metrics={"score": score, "passed": 1.0 if score == 100.0 else 0.0}, provider_run_id=f"qc-{job.id}")
