...
class MediaDocumentWorker(Worker):
    ...
    def execute(self, job: GenerationJob, context: WorkerContext) -> JobExecutionResult:
        ...
        if job.type is JobType.SUBTITLE: result = self._document(job, AssetType.SUBTITLE, "text/vtt; charset=utf-8", self._subtitle(job))
        elif job.type is JobType.METADATA: result = self._document(job, AssetType.DOCUMENT, "application/json; charset=utf-8", self._metadata(job))
        elif job.type is JobType.THUMBNAIL: result = self._thumbnail_asset(job)
        elif job.type is JobType.QC: result = self._final_qc(job)
        else: return JobExecutionResult(False, error_code="UNSUPPORTED_JOB_TYPE", error_message=job.type.value)
        if result.success: self._progress(context, 1.0, "completed")
        return result
    ...
    def _vtt_time(cls, value: Any, default: str) -> str:
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            total = float(value)
            if not math.isfinite(total) or total < 0: raise ValueError("INVALID_SUBTITLE_TIME")
            return f"{int(total // 3600):02d}:{int((total % 3600) // 60):02d}:{total % 60:06.3f}"
        text = str(value or default).strip()
        if len(text) == 5 and text.count(":") == 1: text = "00:" + text + ".000"
        match = cls._VTT_CLOCK.fullmatch(text)
        if not match: raise ValueError("INVALID_SUBTITLE_TIME")
        hours, minutes, seconds = int(match.group(1) or 0), int(match.group(2)), int(match.group(3))
        if minutes >= 60 or seconds >= 60: raise ValueError("INVALID_SUBTITLE_TIME")
        fraction = match.group(4) or ""
        return f"{hours:02d}:{minutes:02d}:{seconds:02d}.{int(fraction.ljust(3, '0')) if fraction else 0:03d}"
    @classmethod
    def _vtt_seconds(cls, value: str) -> float:
        match = cls._VTT_CLOCK.fullmatch(value)
        if not match: raise ValueError("INVALID_SUBTITLE_TIME")
        return int(match.group(1) or 0) * 3600 + int(match.group(2)) * 60 + int(match.group(3)) + int((match.group(4) or "").ljust(3, "0") or 0) / 1000
    @classmethod
    def _subtitle(cls, job: GenerationJob) -> bytes:
        variant = job.input.parameters.get("languagePackVariant")
        language = str(job.input.parameters.get("language") or ((variant or {}).get("language") if isinstance(variant, dict) else ""))
        ...
        if not cues:
            ...
            if text: cues = [{"text": text, "start": 0, "end": job.input.parameters.get("end", "00:00:05.000")}]
        lines = ["WEBVTT", ""]
        for cue in cues:
            ...
            if cls._vtt_seconds(end) <= cls._vtt_seconds(start): raise ValueError("INVALID_SUBTITLE_TIMING")
    ...
    def _run_process(self, job_id: str, args: list[str]) -> tuple[subprocess.CompletedProcess[str] | None, bool]:
        ...
        try:
            with self._process_lock:
                proc = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, start_new_session=True); self._processes[job_id] = proc
            try: stdout, stderr = proc.communicate(timeout=self.subprocess_timeout_seconds)
            except subprocess.TimeoutExpired:
                self._terminate_process(proc); stdout, stderr = proc.communicate(); return subprocess.CompletedProcess(args, proc.returncode, stdout, stderr), True
            return subprocess.CompletedProcess(args, proc.returncode, stdout, stderr), False
        except OSError as exc: return subprocess.CompletedProcess(args, 127, "", str(exc)), False
        finally:
            with self._process_lock:
                if proc is not None and self._processes.get(job_id) is proc: self._processes.pop(job_id, None)
    @staticmethod
    ...
    def _thumbnail_asset(self, job: GenerationJob) -> JobExecutionResult:
        ...
        try: timestamp = self._vtt_time(timestamp, "00:00:01.000")
        except ValueError: return JobExecutionResult(False, error_code="THUMBNAIL_TIMESTAMP_INVALID", error_message="Invalid thumbnail timestamp")
        with tempfile.TemporaryDirectory(prefix="aicf-media-") as temp:
            output_path = Path(temp) / "thumbnail.jpg"; completed, timed_out = self._run_process(job.id, [self.ffmpeg_binary, "-hide_banner", "-loglevel", "error", "-y", "-ss", timestamp, "-i", str(source_path), "-frames:v", "1", "-q:v", "2", str(output_path)])
            if timed_out: return JobExecutionResult(False, error_code="THUMBNAIL_TIMEOUT", error_message="FFmpeg thumbnail extraction timed out", retryable=True)
            if completed is None or completed.returncode != 0 or not output_path.is_file(): return JobExecutionResult(False, error_code="THUMBNAIL_GENERATION_FAILED", error_message=completed.stderr[-2000:] if completed else "FFmpeg process failed", retryable=True)
    ...
    def _final_qc(self, job: GenerationJob) -> JobExecutionResult:
        checks = []; ffprobe_available = shutil.which(self.ffprobe_binary) is not None
        for asset_id in job.input.reference_asset_ids:
            asset = self.assets.get(asset_id); check = {"assetId": asset_id, "exists": asset is not None, "ready": bool(asset and asset.status is AssetStatus.READY), "checksum": bool(asset and asset.sha256)}
            if asset and asset.type is AssetType.VIDEO:
                check["ffprobe"] = False
                if ffprobe_available and Path(asset.path).is_file():
                    probe, timed_out = self._run_process(job.id, [self.ffprobe_binary, "-v", "error", "-print_format", "json", "-show_streams", "-show_format", asset.path]); check["ffprobe"] = bool(probe and probe.returncode == 0)
                    if timed_out: check["error"] = "FFprobe timed out"
                    elif probe and probe.returncode != 0: check["error"] = probe.stderr[-1000:]
        ...
        if not passed: return JobExecutionResult(False, error_code="FINAL_QC_FAILED", error_message=";".join(c.get("error", "INVALID_MEDIA") for c in checks if not (c.get("exists") and c.get("ready") and c.get("checksum") and c.get("ffprobe", True))))
    ...
    def cancel(self, job_id: str) -> None:
        with self._process_lock: proc = self._processes.get(job_id)
        if proc is not None: self._terminate_process(proc)
    def shutdown(self) -> None:
        with self._process_lock: processes = list(self._processes.values())
        for proc in processes: self._terminate_process(proc)
        with self._process_lock: self._processes.clear()
        self._initialized = False
