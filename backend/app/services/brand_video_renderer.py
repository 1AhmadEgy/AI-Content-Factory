...
class BrandVideoRenderer:
    ...
    def _normalize_source(self, source: Path, target: Path, width: int, height: int, fps: int, watermark: Path, opacity: float) -> None:
        video_filter = (
        ...
        args = [
            ...
            "-i", str(source), "-loop", "1", "-i", str(watermark),
            "-filter_complex", filter_complex, "-map", "[vout]",
        ]
        if self._has_audio(source):
            args += ["-map", "0:a:0", "-c:a", "aac", "-b:a", "192k"]
        else:
            args += ["-f", "lavfi", "-i", "anullsrc=channel_layout=stereo:sample_rate=44100", "-map", "2:a:0", "-c:a", "aac", "-b:a", "192k"]
