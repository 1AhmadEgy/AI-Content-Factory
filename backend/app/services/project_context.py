...
class ProjectContextStore:
    ...
    def save(self, project_id: str, context: dict[str, Any], *, event_type: str = "context.updated", entity_type: str | None = None, entity_id: str | None = None) -> dict[str, Any]:
        now = _now()
        with self.store._lock, self.store.connection:
            row = self.store.connection.execute(
                "SELECT version, created_at FROM project_contexts WHERE project_id=?",
                (project_id,),
            ).fetchone()
            ...
            self.store.connection.execute(
                ...
                (f"ctxevt_{uuid4().hex}", project_id, version, event_type, entity_type, entity_id,
                 json.dumps(context, ensure_ascii=False, separators=(",", ":")), now),
            )
    ...
    def merge(self, project_id: str, patch: dict[str, Any], *, event_type: str = "context.merged", entity_type: str | None = None, entity_id: str | None = None) -> dict[str, Any]:
        ...
        for key, value in patch.items():
            if isinstance(value, dict) and isinstance(merged.get(key), dict):
                ...
                merged[key] = value
        return self.save(project_id, merged, event_type=event_type, entity_type=entity_type, entity_id=entity_id)
    ...
    def append_event(self, project_id: str, event_type: str, payload: dict[str, Any], *, entity_type: str | None = None, entity_id: str | None = None) -> dict[str, Any]:
        ...
        return self.save(project_id, current["context"], event_type=event_type, entity_type=entity_type, entity_id=entity_id)

    def events(self, project_id: str, limit: int = 100) -> list[dict[str, Any]]:
        with self.store._lock:
            rows = self.store.connection.execute(
                "SELECT id, version, event_type, entity_type, entity_id, payload_json, created_at FROM project_context_events WHERE project_id=? ORDER BY version DESC LIMIT ?",
                (project_id, max(1, min(limit, 1000))),
            ).fetchall()
        return [
            {
                "id": r["id"], "version": r["version"], "eventType": r["event_type"],
                "entityType": r["entity_type"], "entityId": r["entity_id"],
                "payload": json.loads(r["payload_json"] or "{}"), "createdAt": r["created_at"],
            }
            for r in rows
        ]
