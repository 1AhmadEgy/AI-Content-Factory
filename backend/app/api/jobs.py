...
def build_router(repository: SQLiteJobRepository, runtime: OrchestratorRuntime | None = None, events: SQLiteJobEventRepository | None = None) -> APIRouter:
    ...
    def create_job(request: CreateJobRequest, http_request: Request, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")) -> dict[str, Any]:
        ...
        if result.replayed:
            if result.job is None: raise HTTPException(status_code=409, detail="IDEMPOTENCY_RESOURCE_MISSING")
            return {"data": _serialize(result.job), "requestId": http_request.state.request_id, "idempotentReplay": True}
        if result.job is None: raise HTTPException(status_code=500, detail="JOB_CREATION_FAILED")
        if runtime: runtime.queue.enqueue(result.job)
