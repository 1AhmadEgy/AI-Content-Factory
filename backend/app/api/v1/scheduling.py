...
from ...scheduling.persistent import PersistentScheduler, Schedule, SQLiteScheduleRepository
...
class ScheduleRequest(BaseModel):
    ...
    cron: str | None = None
    intervalSeconds: int | None = Field(default=None, ge=1, le=31536000)
    timezone: str = "UTC"

    @model_validator(mode="after")
    def validate_schedule(self):
        if not self.cron and not self.intervalSeconds and not self.runAt:
            raise ValueError("ONE_OF_RUN_AT_CRON_INTERVAL_REQUIRED")
...
def _serialize(s: Schedule) -> dict[str, Any]:
    return {"id":s.id,"projectId":s.project_id,"operation":s.operation,"payload":s.payload,"runAt":s.run_at.isoformat(),"cron":s.cron,"intervalSeconds":s.interval_seconds,"timezone":s.timezone_name,"enabled":s.enabled,"lastRunAt":s.last_run_at.isoformat() if s.last_run_at else None,"nextRunAt":s.next_run_at.isoformat() if s.next_run_at else None,"createdAt":s.created_at.isoformat(),"updatedAt":s.updated_at.isoformat()}
...
def build_router(projects: SQLiteProjectRepository, schedules: SQLiteScheduleRepository, scheduler: PersistentScheduler) -> APIRouter:
    ...
    def list_schedules(request: Request, project_id: str | None = None, enabled: bool | None = None):
        data=[_serialize(s) for s in schedules.list(project_id,enabled)]
        return {"data":data,"meta":{"count":len(data)},"requestId":request.state.request_id}
    ...
    def create_schedule(body: ScheduleRequest, request: Request):
        ...
        run_at=body.runAt or datetime.now(timezone.utc)
        if run_at.tzinfo is None: run_at=run_at.replace(tzinfo=timezone.utc)
        s=Schedule(id="sch_"+__import__('uuid').uuid4().hex,project_id=body.projectId,operation=body.operation,payload=body.payload,run_at=run_at,cron=body.cron,interval_seconds=body.intervalSeconds,timezone_name=body.timezone,next_run_at=run_at)
        schedules.create(s)
    ...
    def tick(request: Request):
        return {"data":{"createdJobIds":scheduler.tick()},"requestId":request.state.request_id}
