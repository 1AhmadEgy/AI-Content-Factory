...
def _serialize_plan(plan: Any) -> dict[str, Any]:
    return {"title": plan.title, "logline": plan.logline, "synopsis": plan.synopsis, "scenes": [{"number": scene.number, "title": scene.title, "durationSeconds": scene.duration_seconds, "visual": scene.visual, "narration": scene.narration, "shots": [{"number": shot.number, "prompt": shot.prompt, "durationSeconds": shot.duration_seconds, "camera": shot.camera, "lighting": shot.lighting, "style": shot.style} for shot in scene.shots]} for scene in plan.scenes]}
...
def _runtime_error_code(exc: RuntimeError) -> str:
    """Return a stable machine-readable code while keeping provider diagnostics internal."""
    text = str(exc).strip()
    if not text:
        return "AI_PROVIDER_UNAVAILABLE"
    return text.split(":", 1)[0].strip() or "AI_PROVIDER_UNAVAILABLE"
...
def build_router(projects: SQLiteProjectRepository, jobs: SQLiteJobRepository, runtime: OrchestratorRuntime) -> APIRouter:
    ...
    def plan_factory(request: BriefRequest, http_request: Request) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=503, detail=_runtime_error_code(exc)) from exc
    ...
    def start_factory(project_id: str, request: StartFactoryRequest, http_request: Request, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")) -> dict[str, Any]:
        ...
        result = job_service.create_with_idempotency(key=idempotency_key, operation=operation, fingerprint=fingerprint, project_id=project_id, job_type=JobType.STORY, target_type="project", target_id=project_id, priority=100, provider=provider, model=model_id, input=job_input)
        if result.conflict:
