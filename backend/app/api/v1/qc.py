...
def build_router(runtime: OrchestratorRuntime, jobs: SQLiteJobRepository) -> APIRouter:
    ...
    with store._lock, store.connection:
        store.connection.execute(
            "CREATE TABLE IF NOT EXISTS qc_reviews (id TEXT PRIMARY KEY, asset_id TEXT NOT NULL, status TEXT NOT NULL, reason TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)"
        )
        store.connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_qc_reviews_asset ON qc_reviews(asset_id, created_at DESC)"
        )
    ...
    def asset_qc(asset_id: str, request: Request) -> dict:
        ...
        with store._lock:
            rows = store.connection.execute(
                "SELECT * FROM qc_reviews WHERE asset_id=? ORDER BY created_at DESC",
                (asset_id,),
            ).fetchall()
    ...
    def run_qc(asset_id: str, request: Request) -> dict:
        ...
        job = service.create(
            ...
            provider="local",
            model=None,
            input=JobInput(reference_asset_ids=[asset_id]),
        )
    ...
    def approve(qc_id: str, request: Request) -> dict:
        ...
        if row is None:
            raise HTTPException(status_code=404, detail="QC_NOT_FOUND")
        store._insert(
            "UPDATE qc_reviews SET status='APPROVED', updated_at=? WHERE id=?",
            (datetime.now(timezone.utc).isoformat(), qc_id),
        )
    ...
    def reject(qc_id: str, body: RejectRequest, request: Request) -> dict:
        ...
        if row is None:
            raise HTTPException(status_code=404, detail="QC_NOT_FOUND")
        store._insert(
            "UPDATE qc_reviews SET status='REJECTED', reason=?, updated_at=? WHERE id=?",
            (body.reason, datetime.now(timezone.utc).isoformat(), qc_id),
        )
        return {
            "data": {"id": qc_id, "status": "REJECTED", "reason": body.reason},
