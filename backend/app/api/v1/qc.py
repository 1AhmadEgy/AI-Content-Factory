...
def build_router(runtime: OrchestratorRuntime, jobs: SQLiteJobRepository) -> APIRouter:
    ...
    def get_qc(qc_id: str, request: Request) -> dict:
        ...
        if row is None: raise HTTPException(status_code=404, detail="QC_NOT_FOUND")
    ...
    def asset_qc(asset_id: str, request: Request) -> dict:
        if runtime.assets.get(asset_id) is None: raise HTTPException(status_code=404, detail="ASSET_NOT_FOUND")
        with store._lock: rows = store.connection.execute("SELECT * FROM qc_reviews WHERE asset_id=? ORDER BY created_at DESC", (asset_id,)).fetchall()
