...
def build_router(runtime: OrchestratorRuntime, jobs: SQLiteJobRepository) -> APIRouter:
    ...
    def render(body: RenderRequest, request: Request, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key")) -> dict[str, Any]:
        ...
        if runtime.repositories.projects.get(body.projectId) is None: raise HTTPException(status_code=404, detail="PROJECT_NOT_FOUND")
        timeline_asset = runtime.assets.get(body.timelineId)
        if timeline_asset is None or timeline_asset.project_id != body.projectId: raise HTTPException(status_code=404, detail="TIMELINE_NOT_FOUND")
        if timeline_asset.type is not AssetType.DOCUMENT or timeline_asset.status is not AssetStatus.READY: raise HTTPException(status_code=422, detail="TIMELINE_NOT_READY")
        operation = "POST:/api/v1/render"; fingerprint = _fingerprint(body); existing = jobs.store.get_idempotency(idempotency_key, operation)
        if existing:
