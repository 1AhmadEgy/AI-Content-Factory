...
class JobExecutor:
    ...
    def __init__(self, jobs: JobRepository, queue: JobQueue, workers: WorkerRegistry, events: Callable[[JobEvent], None] | None = None, on_completed: Callable[[GenerationJob], None] | None = None, heartbeat_interval_seconds: float | None = None, completion_gate: CompletionGate | None = None) -> None:
        ...
        if configured_interval is None: configured_interval = float(os.getenv("AICF_LEASE_HEARTBEAT_SECONDS", "5.0"))
        if configured_interval <= 0: raise ValueError("heartbeat_interval_seconds must be positive")
    ...
    def execute_claimed(self, job: GenerationJob, lease: JobLease, *, worker_id: str | None = None) -> ExecutionResult:
        ...
        if not worker.health_check(): return self._fail(job, lease, "WORKER_UNHEALTHY", "Worker health check failed", retryable=True)
        ...
        try:
            ...
            heartbeat.stop()
        if not self.queue.is_lease_active(lease): raise RuntimeError("JOB_LEASE_LOST")
        ...
        if not result.asset_ids: return self._fail(job, lease, "MISSING_OUTPUT_ASSET", "Successful worker execution returned no assets", retryable=False)
        ...
        # Persist terminal progress without emitting a second identical progress event;
        # workers are allowed to report their own final progress already.
        job.progress = 1.0
        self._require_persisted(job, lease)
    ...
    def cancel_claimed(self, job: GenerationJob, lease: JobLease, worker_id: str | None = None) -> ExecutionResult:
        if not self.queue.is_lease_active(lease): raise RuntimeError("JOB_LEASE_LOST")
        worker = self.workers.get(worker_id or lease.worker_id); worker.cancel(job.id)
        job.error_code = "CANCELLED"; job.error_message = "Cancellation requested"
        self._require_persisted(job, lease); transition(job, JobStatus.CANCELLED); self.queue.acknowledge(lease, JobStatus.CANCELLED); self._event(job, "JOB_CANCELLED", {})
    ...
    def _set_progress(self, job: GenerationJob, stage: str, progress: float, *, persist: bool = True, lease: JobLease | None = None) -> None:
        if lease is not None and not self.queue.is_lease_active(lease): return
        job.progress = max(0.0, min(1.0, float(progress)))
        if persist:
            if lease is not None:
                if not self._persist_claimed(job, lease): return
            else: self.jobs.update(job)
    ...
    def _fail(self, job: GenerationJob, lease: JobLease, code: str, message: str, retryable: bool) -> ExecutionResult:
        if not self.queue.is_lease_active(lease): raise RuntimeError("JOB_LEASE_LOST")
        job.error_code = code; job.error_message = message
        if retryable and job.attempt < job.max_attempts:
            self._require_persisted(job, lease); transition(job, JobStatus.RETRYING); self.queue.acknowledge(lease, JobStatus.RETRYING); transition(job, JobStatus.QUEUED)
        ...
        self._require_persisted(job, lease); transition(job, JobStatus.FAILED); self.queue.acknowledge(lease, JobStatus.FAILED); self._event(job, "JOB_FAILED", {"errorCode": code, "retryable": retryable, "attempt": job.attempt})
        return ExecutionResult(job, JobStatus.FAILED)

    def _persist_claimed(self, job: GenerationJob, lease: JobLease) -> bool:
        if not self.queue.is_lease_active(lease): return False
        update_if_current = getattr(self.jobs, "update_if_current", None)
        if update_if_current is not None: return bool(update_if_current(job, JobStatus.RUNNING, job.attempt))
        self.jobs.update(job); return True

    def _require_persisted(self, job: GenerationJob, lease: JobLease) -> None:
        if not self._persist_claimed(job, lease): raise RuntimeError("JOB_LEASE_LOST")

    def _notify_completed(self, job: GenerationJob) -> None:
        try: self.on_completed(job)
        except Exception: logger.exception("Post-completion callback failed for job %s", job.id)

    def _event(self, job: GenerationJob, event_type: str, payload: dict[str, object]) -> None:
        try: self.emit(JobEvent.create(job.id, job.project_id, event_type, job.status.value, job.progress, payload))
        except Exception: logger.exception("Job event emission failed for %s (%s)", job.id, event_type)
