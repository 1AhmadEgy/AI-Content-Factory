...
class ContentPipelineOrchestrator:
    ...
    def on_completed(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        if job.type is JobType.SUBTITLE: return self._maybe_create_timeline_for_scene(job)
    ...
    def _create_generation_jobs(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        if not isinstance(scene_data, dict): scene_data = {"number": job.input.parameters.get("sceneNumber", 1), "title": "Scene", "durationSeconds": shot_data.get("durationSeconds", 5), "visual": shot_data.get("prompt", ""), "narration": ""}
    ...
    def _maybe_create_best_take(self, qc_job: GenerationJob) -> list[GenerationJob]:
        ...
        for generation in generations:
            ...
            if not qc_jobs or not generation.output: continue
    ...
    def _maybe_create_timeline_for_scene(self, completed_job: GenerationJob) -> list[GenerationJob]:
        ...
        if narration and not audio_jobs: return []
        ...
        if narration and subtitle is None:
            ...
            if duration <= 0 or not audio_jobs or not audio_jobs[0].output or not audio_jobs[0].output.asset_ids: return []
            subtitle_parameters = {"language": str(scene.input.parameters.get("brief", {}).get("language", "en")), "text": narration, "end": duration, "sceneJobId": scene.id, "sourceTtsJobId": audio_jobs[0].id, "subtitleStage": "timeline_input"}
            self._enqueue(scene, JobType.SUBTITLE, "subtitle", f"{scene.id}:subtitle", subtitle_parameters, 5, [audio_jobs[0].output.asset_ids[0]])
            return []
        image_ids: list[str] = []
        ...
        try: duration_us = int(float(scene_data.get("durationSeconds", 5)) * 1_000_000)
        except (TypeError, ValueError, OverflowError): return []
        if duration_us <= 0: return []
        refs = [*image_ids, *audio_ids]
        parameters = {"durationUs": duration_us, "sceneJobId": scene.id, "videoAssetIds": image_ids, "audioAssetIds": audio_ids, "subtitleAssetId": subtitle.output.asset_ids[0] if subtitle else None, "sourceAssetTypes": ["IMAGE", "AUDIO"] + (["SUBTITLE"] if subtitle else [])}
        if subtitle: refs.append(subtitle.output.asset_ids[0])
    ...
    def _create_render_job(self, job: GenerationJob) -> list[GenerationJob]:
        if not job.output or not job.output.asset_ids: return []
        parameters = {"resolution": job.input.parameters.get("resolution", "1080p"), "aspectRatio": job.input.parameters.get("aspectRatio", "16:9"), "fps": job.input.parameters.get("fps", 30), "subtitleAssetId": job.input.parameters.get("subtitleAssetId"), "language": job.input.parameters.get("language"), "locale": job.input.parameters.get("locale"), "languagePackVersion": job.input.parameters.get("languagePackVersion")}
