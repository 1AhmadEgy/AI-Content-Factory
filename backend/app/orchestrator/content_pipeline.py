...
class ContentPipelineOrchestrator:
    ...
    def _create_generation_jobs(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        count=max(1,min(int(job.input.parameters.get("takeCount",4)),8)); created=[]
    ...
    def _maybe_create_best_take(self, qc_job: GenerationJob) -> list[GenerationJob]:
        ...
        return [self._enqueue(shot,JobType.BEST_TAKE,"best_take",f"{shot.id}:best-take",{"candidates":candidates,"shotJobId":shot.id,"takeCount":shot.input.parameters.get("takeCount",1)},5)] if candidates else []
    ...
    def _maybe_create_timeline_for_scene(self, completed_job: GenerationJob) -> list[GenerationJob]:
        ...
        refs=[j.output.asset_ids[0] for j in best]; refs.extend(qc.output.asset_ids[0] for qc in audio_qcs)
    ...
    def _enqueue(self, parent: GenerationJob, job_type: JobType, target_type: str, target_id: str, parameters: dict[str, object], priority_offset: int, reference_asset_ids: list[str] | None = None) -> GenerationJob:
        ...
        create_with_idempotency=getattr(self.job_service,"create_with_idempotency",None)
        if create_with_idempotency is None:
            child=self.job_service.create(project_id=parent.project_id,job_type=job_type,target_type=target_type,target_id=target_id,parent_job_id=parent.id,priority=priority,provider=parent.provider,model=parent.model,input=input_data)
            self.enqueue(child); return child
        ...
        result=create_with_idempotency(key=key,operation="pipeline_enqueue",fingerprint=fingerprint,project_id=parent.project_id,job_type=job_type,target_type=target_type,target_id=target_id,parent_job_id=parent.id,input=input_data,priority=priority,max_attempts=3,provider=parent.provider,model=parent.model)
        if hasattr(result,"conflict"):
            if result.conflict: raise ValueError(f"PIPELINE_IDEMPOTENCY_CONFLICT: {key}")
            child=result.job
            if child is None and result.existing_resource_id: child=self.job_service.repository.get(result.existing_resource_id)
        else:
            child,existing_resource_id=result
            if existing_resource_id == "__IDEMPOTENCY_CONFLICT__": raise ValueError(f"PIPELINE_IDEMPOTENCY_CONFLICT: {key}")
            if child is None and existing_resource_id: child=self.job_service.repository.get(existing_resource_id)
        if child is None: raise RuntimeError(f"PIPELINE_IDEMPOTENCY_RESOURCE_MISSING: {key}")
        if child.status is JobStatus.QUEUED: self.enqueue(child)
        return child
