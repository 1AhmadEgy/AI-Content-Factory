...
class ContentPipelineOrchestrator:
    """Expand jobs through the complete production lifecycle with dependency-aware delivery."""
    ...
    def on_completed(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        if job.type is JobType.SUBTITLE:
            return self._create_thumbnail_job(job)
        if job.type is JobType.THUMBNAIL:
            return self._create_metadata_job(job)
        if job.type is JobType.METADATA:
            return self._create_publish_job(job)
        if job.type is JobType.PUBLISH:
            return self._create_repurpose_job(job)
        return []
    ...
    def _maybe_create_best_take(self, qc_job: GenerationJob) -> list[GenerationJob]:
        ...
        if len(videos) < int(shot.input.parameters.get("takeCount", 1)):
            return []
    ...
    def _delivery_common(self, job: GenerationJob) -> dict[str, object]:
        params = job.input.parameters
        return {
            "title": params.get("title", "AI Content"),
            "description": params.get("description", ""),
            "language": params.get("language", "en"),
            "platforms": params.get("platforms", ["youtube", "tiktok", "instagram", "facebook"]),
    ...
    def _create_delivery_jobs(self, job: GenerationJob) -> list[GenerationJob]:
        if not job.input.reference_asset_ids:
            return []
        return [self._enqueue(job, JobType.SUBTITLE, "subtitle", f"{job.id}:subtitle", {**self._delivery_common(job), "deliveryRootJobId": job.id}, 1, list(job.input.reference_asset_ids))]

    def _create_thumbnail_job(self, job: GenerationJob) -> list[GenerationJob]:
        if not job.output or not job.output.asset_ids:
            return []
        return [self._enqueue(job, JobType.THUMBNAIL, "thumbnail", f"{job.id}:thumbnail", {**self._delivery_common(job), "deliveryRootJobId": job.input.parameters.get("deliveryRootJobId", job.parent_job_id)}, 2, list(job.output.asset_ids))]

    def _create_metadata_job(self, job: GenerationJob) -> list[GenerationJob]:
        if not job.output or not job.output.asset_ids:
            return []
        return [self._enqueue(job, JobType.METADATA, "metadata", f"{job.id}:metadata", {**self._delivery_common(job), "deliveryRootJobId": job.input.parameters.get("deliveryRootJobId", job.parent_job_id)}, 3, list(job.output.asset_ids))]

    def _create_publish_job(self, job: GenerationJob) -> list[GenerationJob]:
        if not job.output or not job.output.asset_ids:
            return []
        return [self._enqueue(job, JobType.PUBLISH, "publish", f"{job.id}:publish", {**self._delivery_common(job), "deliveryRootJobId": job.input.parameters.get("deliveryRootJobId", job.parent_job_id)}, 4, list(job.output.asset_ids))]

    def _create_repurpose_job(self, job: GenerationJob) -> list[GenerationJob]:
        if not job.output or not job.output.asset_ids:
            return []
        return [self._enqueue(job, JobType.REPURPOSE, "repurpose", f"{job.id}:repurpose", {**self._delivery_common(job), "deliveryRootJobId": job.input.parameters.get("deliveryRootJobId", job.parent_job_id)}, 5, list(job.output.asset_ids))]
