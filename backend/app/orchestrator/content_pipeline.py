...
class ContentPipelineOrchestrator:
    ...
    def _create_scene_jobs(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        brief = {
            k: job.input.parameters[k]
            for k in ("topic", "language", "durationSeconds", "style", "audience", "platform", "aspectRatio")
            if k in job.input.parameters
        }
        return [
            self._enqueue(
                job,
                JobType.SCENE,
                "scene",
                f"{job.id}:scene:{int(scene.get('number', i))}",
                {"scene": scene, "brief": brief, "storyJobId": job.id, "sceneNumber": int(scene.get("number", i))},
                1,
            )
    ...
    def _create_shot_jobs(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        return [
            self._enqueue(
                ...
                JobType.SHOT,
                "shot",
                f"{scene_id}:shot:{int(shot.get('number', i))}",
                {
                    "shot": shot,
                    "scene": scene,
                    "brief": job.input.parameters.get("brief", {}),
                    "sceneJobId": job.id,
                    "sceneNumber": job.input.parameters.get("sceneNumber"),
                    "shotNumber": int(shot.get("number", i)),
                    "takeCount": job.input.parameters.get("takeCount", 4),
                },
                2,
            )
    ...
    def _create_audio_jobs(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        brief = ContentBrief(
            ...
            duration_seconds=int(brief_data.get("durationSeconds", 60)),
        ...
        scene = ScenePlan(
            number=int(scene_data.get("number", 1)),
        ...
        return [
            self._enqueue(
                job,
                spec.job_type,
                spec.job_type.value.lower(),
                f"{job.id}:{spec.job_type.value.lower()}",
                params,
                2,
            )
        ]
    ...
    def _create_generation_jobs(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        if not isinstance(scene_data, dict):
            scene_data = {
                "number": job.input.parameters.get("sceneNumber", 1),
                "title": "Scene",
                "durationSeconds": shot_data.get("durationSeconds", 5),
                "visual": shot_data.get("prompt", ""),
                "narration": "",
            }
        brief_data = job.input.parameters.get("brief", {})
        ...
        scene = ScenePlan(
            number=int(scene_data.get("number", 1)),
            title=str(scene_data.get("title", "Scene")),
            duration_seconds=float(scene_data.get("durationSeconds", 5)),
            visual=str(scene_data.get("visual", "")),
            narration=str(scene_data.get("narration", "")),
            shots=[],
        )
        shot = ShotPlan(
            number=int(shot_data.get("number", 1)),
            prompt=str(shot_data.get("prompt", "")),
            duration_seconds=float(shot_data.get("durationSeconds", 5)),
    ...
    def _create_qc_job(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        params = {
            "sourceJobId": job.id,
            "takeNumber": job.input.parameters.get("takeNumber"),
            "takeCount": job.input.parameters.get("takeCount", 1),
            "takeGroupId": job.input.parameters.get("takeGroupId"),
            "auxiliary": bool(job.input.parameters.get("auxiliary")),
        }
    ...
    def _maybe_create_best_take(self, qc_job: GenerationJob) -> list[GenerationJob]:
        ...
        for generation in generations:
            qc_jobs = [
                q
                for q in self.job_service.repository.list_by_parent(generation.id)
                if q.type is JobType.QC and q.status is JobStatus.COMPLETED and q.output and q.output.asset_ids
            ]
            if not qc_jobs or not generation.output:
                continue
            score = float((qc_jobs[0].output.metrics if qc_jobs[0].output else {}).get("score", 0))
            candidates.extend(
                {"assetId": asset_id, "score": score, "takeNumber": generation.input.parameters.get("takeNumber")}
                for asset_id in generation.output.asset_ids
            )
        if not candidates:
            return []
        return [
            self._enqueue(
                ...
                "best_take",
                f"{shot.id}:best-take",
                {"candidates": candidates, "shotJobId": shot.id, "takeCount": shot.input.parameters.get("takeCount", 1)},
                5,
            )
        ]
    ...
    def _maybe_create_timeline_for_scene(self, completed_job: GenerationJob) -> list[GenerationJob]:
        ...
        best = [
            j
            for shot in shots
            for j in self.job_service.repository.list_by_parent(shot.id)
            if j.type is JobType.BEST_TAKE and j.status is JobStatus.COMPLETED and j.output and j.output.asset_ids
        ]
        if not shots or len(best) < len(shots):
            return []
        audio_jobs = [
            j
            for j in self.job_service.repository.list_by_parent(scene.id)
            if j.type is JobType.TTS and j.input.parameters.get("auxiliary")
        ]
        ...
        if narration and not audio_jobs:
            return []
        subtitle_jobs = [
            j
            for j in self.job_service.repository.list_by_parent(scene.id)
            if j.type is JobType.SUBTITLE and not j.input.parameters.get("finalDelivery")
        ]
        ...
        if narration and subtitle is None:
            ...
            if duration <= 0 or not audio_jobs or not audio_jobs[0].output or not audio_jobs[0].output.asset_ids:
                return []
            brief_data = scene.input.parameters.get("brief", {})
            if not isinstance(brief_data, dict):
                brief_data = {}
            subtitle_parameters = {
                "language": str(brief_data.get("language", "en")),
                ...
                "end": duration,
                "sceneJobId": scene.id,
                "sourceTtsJobId": audio_jobs[0].id,
            ...
            self._enqueue(
                ...
                subtitle_parameters,
                5,
                [audio_jobs[0].output.asset_ids[0]],
            )
            return []
        image_ids: list[str] = []
    ...
    def _create_final_qc_job(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        return [
            self._enqueue(
                ...
                "final_qc",
                f"{job.id}:final-qc",
                {"sourceJobId": job.id, "finalQc": True},
                8,
                list(job.output.asset_ids),
            )
        ]
    ...
    def _create_delivery_jobs(self, job: GenerationJob) -> list[GenerationJob]:
        ...
        jobs = [
            self._enqueue(job, t, target, f"{job.id}:{target}", common, 1, ids)
            for t, target in (
                (JobType.SUBTITLE, "subtitle"),
                (JobType.THUMBNAIL, "thumbnail"),
                (JobType.METADATA, "metadata"),
                (JobType.REPURPOSE, "repurpose"),
            )
        ]
        if common["platforms"]:
